...
type incusLeaseTransport struct {
	t                *testing.T
	stateUnavailable bool
}

func (tr incusLeaseTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	var payload string
	...
	case "/1.0/instances/guest":
		payload = `{"metadata":{"status":"Running","config":{"volatile.uplink.hwaddr":"00:16:3e:11:22:33"},"expanded_devices":{"uplink":{"type":"nic","network":"incusbr0","name":"enp5s0"}},"devices":{},"profiles":["default"]}}`
	case "/1.0/networks/incusbr0/leases":
...
func TestIncusVMIPv4UsesMatchingBridgeLeaseWithoutGuestAgent(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			p.apiClient = &http.Client{Transport: incusLeaseTransport{t: t, stateUnavailable: tc.stateUnavailable}}

...
func (i *IncusProvider) preflightIPv6Network(ctx context.Context, config provider.InstanceConfig, networkConfig NetworkConfig) error {
	if ctx.Err() != nil {
		return ctx.Err()
	}
	...
	if present {
		if output, checkErr := i.sshClient.Execute(routed.HostCheckCommand()); checkErr != nil {
	...
	if err := utils.EnsureIPv6HostEgress(i.sshClient, utils.IPv6EgressOptions{PersistRA: true}); err != nil {

...
func (e *deleteFirewallFailure) ExecuteWithTimeout(command string, _ time.Duration) (string, error) {
	...
	if strings.Contains(command, "kubectl get deploy") {
...
func TestDeleteKeepsInstanceWhenFirewallCleanupFails(t *testing.T) {
	...
	p := &KubeVirtProvider{connected: true, sshClient: utils.NewSafeShellExecutor(executor)}
	if err := p.sshDeleteInstance(context.Background(), "guest"); err == nil {

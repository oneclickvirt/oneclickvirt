...
func (p *ProxmoxProvider) apiListImages(ctx context.Context) ([]provider.Image, error) {
	url := p.apiEndpoint(fmt.Sprintf("/api2/json/nodes/%s/storage/local/content", p.nodeName()))
...
func (p *ProxmoxProvider) apiDeleteImage(ctx context.Context, id string) error {
	url := p.apiEndpoint(fmt.Sprintf("/api2/json/nodes/%s/storage/local/content/%s", p.nodeName(), id))

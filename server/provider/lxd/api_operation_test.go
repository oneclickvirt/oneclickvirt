...
type lxdListResponseTransport struct {
	status int
	body   string
}

func (tr lxdListResponseTransport) RoundTrip(req *http.Request) (*http.Response, error) {
...
func TestLXDAPIListInstancesRejectsNonSuccessAndMalformedMetadata(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			p.apiClient = &http.Client{Transport: lxdListResponseTransport{status: tt.status, body: tt.body}}
...
type lxdIPv4MetadataTransport struct {
	t                 *testing.T
	requests          []string
...
func (tr *lxdIPv4MetadataTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	case "GET /1.0/instances/guest":
		...
		payload = "{\"metadata\":{\"architecture\":\"x86_64\",\"config\":{\"security.nesting\":\"true\",\"limits.memory\":\"512MiB\",\"volatile.uplink.hwaddr\":\"00:16:3e:11:22:33\"},\"expanded_devices\":{\"uplink\":{\"type\":\"nic\",\"network\":\"lxdbr0\",\"name\":\"eth0\",\"mtu\":\"1400\",\"limits.ingress\":\"10Mbit\"}},\"devices\":{\"root\":{\"type\":\"disk\",\"path\":\"/\",\"pool\":\"local\"}},\"profiles\":[\"default\",\"custom\"],\"description\":\"keep me\",\"ephemeral\":false,\"stateful\":false}}"
	...
	case "PUT /1.0/instances/guest":
		...
		if nic["type"] != "nic" || nic["ipv4.address"] != "192.0.2.10" || nic["mtu"] != "1400" || nic["limits.ingress"] != "10Mbit" || nic["network"] != "lxdbr0" {
...
func TestLXDAPIWaitForInstanceIPv4UsesGuestStateAddress(t *testing.T) {
	p := NewLXDProvider().(*LXDProvider)
	p.config = provider.NodeConfig{Host: "127.0.0.1"}

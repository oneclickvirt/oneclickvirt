...
func TestFirewallIntegrationLXDDualStackRetriesAndRemoval(t *testing.T) {
	executor := testutil.NewFirewallExecutor(t)
	l := &LXDProvider{connected: true, config: provider.NodeConfig{ExecutionRule: "ssh_only"}, sshClient: utils.NewSafeShellExecutor(executor)}
	for attempt := 0; attempt < 2; attempt++ {
		for _, address := range []string{"192.0.2.10", "2001:db8::10"} {
			if err := l.setupIptablesMappingWithIP("guest", 22000, 22, "both", address); err != nil {
				t.Fatal(err)
			}
		}
	}
	count := func(ipv6 bool) int {
		command := "nft list table ip lxd"
		if ipv6 {
	...
	if err := l.RemovePortMappingForFamily("guest", 22000, 22, 22000, 22, 1, "both", "iptables", "", true); err != nil {
		t.Fatal(err)
	}
	if count(false) != 2 || count(true) != 0 {
		t.Fatal("IPv6 cleanup affected IPv4 or left stale rules")
	}
	if err := l.RemovePortMappingForFamily("guest", 22000, 22, 22000, 22, 1, "both", "iptables", "", false); err != nil {
		t.Fatal(err)
	}
	if count(false) != 0 {
	...
	if err := l.setupIptablesMappingWithIP("replacement", 22000, 22, "both", "192.0.2.20"); err != nil {
		t.Fatal(err)
	}
	rules := executor.MustExecute(t, "nft list table ip lxd")
...
func TestFirewallIntegrationLXDFailedReadStopsReplacement(t *testing.T) {
	executor := testutil.NewFirewallExecutor(t)
	l := &LXDProvider{connected: true, config: provider.NodeConfig{ExecutionRule: "ssh_only"}, sshClient: utils.NewSafeShellExecutor(executor)}
	if err := l.setupIptablesMappingWithIP("guest", 22000, 22, "tcp", "192.0.2.10"); err != nil {
		t.Fatal(err)
	}
	l.sshClient = utils.NewSafeShellExecutor(&lxdFirewallReadFailure{FirewallExecutor: executor})
	if err := l.setupIptablesMappingWithIP("guest", 22000, 22, "tcp", "192.0.2.11"); err == nil {
...
type lxdFirewallReadFailure struct{ *testutil.FirewallExecutor }

func (e *lxdFirewallReadFailure) ExecuteWithTimeout(command string, timeout time.Duration) (string, error) {

...
func TestLXDAPICreateResourceContract(t *testing.T) {
	...
	for _, kind := range []string{"container", "vm"} {
		for _, tc := range []struct {
			...
			t.Run(kind+"/"+tc.name, func(t *testing.T) {
				...
				p.sshClient = utils.NewSafeShellExecutor(&recordingLXDIPv6Executor{outputs: []string{"cached-fingerprint", "yes"}})
...
func TestLXDVMSecurityDoesNotSetContainerLimits(t *testing.T) {

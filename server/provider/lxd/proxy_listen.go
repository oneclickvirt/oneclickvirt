...
func (l *LXDProvider) getNATProxyListenIP(ctx context.Context, ipv6 bool) (string, error) {
	resolveCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	if address, err := utils.ResolveNATProxyListenIP(resolveCtx, ipv6, l.config.PortIP, l.config.Host); err == nil {
		return address, nil
	}
	...
	client := l.sshClient
	l.mu.RUnlock()
	if client != nil && client.HasExecutor() && !strings.EqualFold(strings.TrimSpace(l.config.ExecutionRule), "api_only") {
	...
	if l.apiClient != nil {
		...
		defer stop()
		req, err := http.NewRequestWithContext(discoveryCtx, http.MethodGet, l.apiEndpoint("/1.0"), nil)
		if err != nil {
			return "", err
		}

export default {
  login: '登录',
  logout: '退出登录',
  register: '注册',
  submit: '提交',
  cancel: '取消',
  next: '下一步',
  confirm: '确定',
  ok: '确定',
  save: '保存',
  saveSuccess: '保存成功',
  saveFailed: '保存失败',
  update: '更新',
  delete: '删除',
  edit: '编辑',
  add: '添加',
  create: '创建',
  search: '搜索',
  reset: '重置',
  back: '返回',
  close: '关闭',
  selected: '已选择',
  loading: '加载中...',
  loadFailed: '加载失败',
  success: '成功',
  error: '错误',
  warning: '警告',
  info: '信息',
  yes: '是',
  no: '否',
  refresh: '刷新',
  copy: '复制',
  paste: '粘贴',
  download: '下载',
  upload: '上传',
  view: '查看',
  export: '导出',
  import: '导入',
  actions: '操作',
  status: '状态',
  name: '名称',
  description: '描述',
  createdAt: '创建时间',
  createTime: '创建时间',
  updatedAt: '更新时间',
  all: '全部',
  selectAll: '全选',
  clear: '清空',
  filter: '筛选',
  more: '更多',
  total: '共',
  enable: '启用',
  disable: '禁用',
  enabled: '已启用',
  disabled: '已禁用',
  partial: '部分可用',
  normal: '正常',
  start: '启动',
  stop: '停止',
  restart: '重启',
  details: '详情',
  settings: '设置',
  apply: '应用',
  approve: '通过',
  reject: '拒绝',
  pending: '待处理',
  processing: '处理中',
  completed: '已完成',
  failed: '失败',
  running: '运行中',
  stopped: '已停止',
  unknown: '未知',
  user: '用户',
  admin: '管理员',
  normalUser: '普通用户',
  notSet: '未设置',
  questionMark: '吗？',
  comma: '，',
  period: '。',
  ellipsis: '...',
  leftParen: '（',
  rightParen: '）',
  inLocation: '中查看',
  backToHome: '返回首页',
  seconds: '秒',
  minutes: '分钟',
  hours: '小时',
  core: '核',
  days: '天',
  presetValue: '预设值，不可修改',
  noData: '暂无数据',
  loginExpired: '登录已过期，请重新登录',
  loginInvalid: '您的登录状态已失效，请重新登录',
  noPermission: '您没有权限执行此操作',
  accountDisabled: '您的账户已被禁用，请联系管理员',
  requestTooLarge: '上传的文件或数据过大，请减少文件大小或数据量',
  apiVersionMismatchTitle: '主控版本不一致',
  apiVersionMismatchMessage: '当前前端与主控的 API 契约不一致，或请求接口未在主控中注册。请完整更新主控二进制和 Web 文件到同一版本后刷新页面，避免仅替换其中一部分。',
  backendVersion: '主控版本',
  nothingToCopy: '没有可复制的内容',
  copySuccess: '复制成功',
  copyFailed: '复制失败，请手动复制',
  pasteFailed: '粘贴失败，请检查浏览器剪贴板权限',
  clipboardUnavailable: '浏览器剪贴板读取不可用，请使用 HTTPS 并允许剪贴板权限',
  connectionClosed: '连接已断开',
  cannotSerialize: '无法序列化对象',
  unknownError: '未知错误',
  popupBlocked: '无法打开新窗口，请检查浏览器弹窗设置',
  announcement: '公告',
  noLogContent: '暂无日志内容',
  cancelled: '已取消',
  items: '个',
  justNow: '刚刚',
  minutesAgo: '{n}分钟前',
  hoursAgo: '{n}小时前',
  daysAgo: '{n}天前',
  zeroDuration: '0秒',
  durationDaysHours: '{days}天{hours}小时',
  durationHoursMinutes: '{hours}小时{minutes}分钟',
  durationMinutesSeconds: '{minutes}分钟{seconds}秒',
  durationSeconds: '{n}秒',
  online: '在线',
  offline: '离线',
  instanceDetail: '实例详情',
  operationSuccess: '操作成功',
  confirmDeleteMessage: '确定要删除吗？此操作不可撤销。',
  deleteSuccess: '删除成功',
  batchConfirmMessage: '确定要处理 {n} 个项目吗？',
  batchCompleted: '批量操作完成',
  getUserInfoFailed: '获取用户信息失败',
  requestFailed: '请求失败',
  operationFailed: '操作失败',
  deleteFailed: '删除失败',
  updateSuccess: '更新成功',
  createSuccess: '创建成功',
  action: '操作',
  size: '大小',
  modifiedTime: '修改时间',
  terminalTab: '终端',
  parentDirectory: '上级目录',
  sftpListFailed: '读取目录失败',
  sftpDownloadFailed: '下载失败',
  uploadSucceeded: '上传成功',
  uploadFailed: '上传失败',
  uploadAlreadyCompleted: '文件已上传完成',
  resumeUploadTitle: '继续上传',
  resumeUploadConfirm: '检测到未完成的上传数据，是否从断点继续？',
  resume: '继续',
  restartUpload: '重新开始',
  question: '提示',
  unsavedChanges: '有未保存的更改',
  unsavedChangesConfirm: '当前表单有未保存的更改，确定要关闭吗？',
  discardChanges: '放弃更改',
  fileTransfer: '文件传输',
  fileTransferTab: '文件传输',
  agentFMNotAvailable: '此节点暂未在线，无法使用文件传输',
  agentFMUnavailableTitle: '文件传输不可用',
  agentFMUnavailableTip: '此节点未设置 SSH 凭据，且 Agent 当前未在线，无法进行文件传输。',
  agentFMUploadSuccess: '文件上传成功',
  agentFMUploadFailed: '文件上传失败',
  agentFMDownloadFailed: '文件下载失败',
  agentFMListFailed: '获取文件列表失败',
  agentFMDeleteSuccess: '删除成功',
  agentFMDeleteFailed: '删除失败',
  agentFMDeleteConfirm: '确定要删除 {name} 吗？此操作不可撤销。',
  agentFMMkdirFailed: '创建目录失败',
  agentFMFileSizeLimit: '文件过大（最大 50 MB），请通过其他方式传输'
}

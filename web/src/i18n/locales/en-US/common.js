export default {
  login: 'Login',
  logout: 'Logout',
  register: 'Register',
  submit: 'Submit',
  cancel: 'Cancel',
  next: 'Next',
  confirm: 'Confirm',
  ok: 'OK',
  save: 'Save',
  saveSuccess: 'Saved successfully',
  saveFailed: 'Save failed',
  update: 'Update',
  delete: 'Delete',
  edit: 'Edit',
  add: 'Add',
  create: 'Create',
  search: 'Search',
  reset: 'Reset',
  back: 'Back',
  close: 'Close',
  selected: 'selected',
  loading: 'Loading...',
  loadFailed: 'Load failed',
  success: 'Success',
  error: 'Error',
  warning: 'Warning',
  info: 'Info',
  yes: 'Yes',
  no: 'No',
  refresh: 'Refresh',
  copy: 'Copy',
  paste: 'Paste',
  download: 'Download',
  upload: 'Upload',
  view: 'View',
  export: 'Export',
  import: 'Import',
  actions: 'Actions',
  status: 'Status',
  name: 'Name',
  description: 'Description',
  createdAt: 'Created At',
  createTime: 'Create Time',
  days: 'days',
  updatedAt: 'Updated At',
  all: 'All',
  selectAll: 'Select All',
  clear: 'Clear',
  filter: 'Filter',
  hours: 'hours',
  minutes: 'minutes',
  more: 'More',
  total: 'Total',
  enable: 'Enable',
  disable: 'Disable',
  enabled: 'Enabled',
  disabled: 'Disabled',
  partial: 'Partially Available',
  normal: 'Normal',
  start: 'Start',
  stop: 'Stop',
  restart: 'Restart',
  details: 'Details',
  settings: 'Settings',
  apply: 'Apply',
  approve: 'Approve',
  reject: 'Reject',
  pending: 'Pending',
  processing: 'Processing',
  completed: 'Completed',
  failed: 'Failed',
  running: 'Running',
  stopped: 'Stopped',
  unknown: 'Unknown',
  user: 'User',
  admin: 'Administrator',
  normalUser: 'Normal User',
  notSet: 'Not Set',
  questionMark: '?',
  comma: ', ',
  period: '. ',
  ellipsis: '...',
  leftParen: ' (',
  rightParen: ')',
  inLocation: ' in',
  backToHome: 'Back to Home',
  core: ' Core',
  presetValue: 'Preset value, cannot be modified',
  noData: 'No data available',
  loginExpired: 'Your login session has expired, please log in again',
  loginInvalid: 'Your login status has become invalid, please log in again',
  noPermission: 'You do not have permission to perform this action',
  accountDisabled: 'Your account has been disabled, please contact the administrator',
  requestTooLarge: 'The uploaded file or data is too large, please reduce the size',
  apiVersionMismatchTitle: 'Controller Version Mismatch',
  apiVersionMismatchMessage: 'The frontend and controller API contracts do not match, or the requested route is not registered. Update the controller binary and web assets to the same release, then refresh the page.',
  backendVersion: 'Controller version',
  nothingToCopy: 'Nothing to copy',
  copySuccess: 'Copied',
  copyFailed: 'Copy failed, please copy manually',
  pasteFailed: 'Paste failed, please check the browser clipboard permission',
  clipboardUnavailable: 'Clipboard reading is unavailable. Use HTTPS and allow clipboard permission',
  connectionClosed: 'Connection closed',
  cannotSerialize: 'Unable to serialize object',
  unknownError: 'Unknown error',
  popupBlocked: 'Unable to open new window, please check browser popup settings',
  seconds: 'seconds',
  announcement: 'Announcement',
  noLogContent: 'No log content available',
  cancelled: 'Cancelled',
  items: '',
  justNow: 'just now',
  minutesAgo: '{n} min ago',
  hoursAgo: '{n} hours ago',
  daysAgo: '{n} days ago',
  zeroDuration: '0s',
  durationDaysHours: '{days}d {hours}h',
  durationHoursMinutes: '{hours}h {minutes}m',
  durationMinutesSeconds: '{minutes}m {seconds}s',
  durationSeconds: '{n}s',
  online: 'Online',
  offline: 'Offline',
  instanceDetail: 'Instance Details',
  operationSuccess: 'Operation successful',
  confirmDeleteMessage: 'Are you sure you want to delete? This action cannot be undone.',
  deleteSuccess: 'Deleted successfully',
  batchConfirmMessage: 'Are you sure you want to process {n} items?',
  batchCompleted: 'Batch operation completed',
  getUserInfoFailed: 'Failed to get user information',
  requestFailed: 'Request failed',
  operationFailed: 'Operation failed',
  deleteFailed: 'Delete failed',
  updateSuccess: 'Updated successfully',
  createSuccess: 'Created successfully',
  action: 'Action',
  size: 'Size',
  modifiedTime: 'Modified',
  terminalTab: 'Terminal',
  parentDirectory: 'Parent',
  sftpListFailed: 'Failed to list directory',
  sftpDownloadFailed: 'Download failed',
  uploadSucceeded: 'Upload succeeded',
  uploadFailed: 'Upload failed',
  uploadAlreadyCompleted: 'Upload already completed',
  resumeUploadTitle: 'Resume Upload',
  resumeUploadConfirm: 'Detected incomplete upload data. Continue from the breakpoint?',
  resume: 'Resume',
  restartUpload: 'Restart',
  question: 'Prompt',
  unsavedChanges: 'Unsaved Changes',
  unsavedChangesConfirm: 'There are unsaved changes in the form. Are you sure you want to close?',
  discardChanges: 'Discard Changes',
  fileTransfer: 'File Transfer',
  fileTransferTab: 'File Transfer',
  agentFMNotAvailable: 'This node is offline, file transfer is unavailable',
  agentFMUnavailableTitle: 'File Transfer Unavailable',
  agentFMUnavailableTip: 'This node has no SSH credentials configured and the Agent is offline. File transfer is unavailable.',
  agentFMUploadSuccess: 'File uploaded successfully',
  agentFMUploadFailed: 'File upload failed',
  agentFMDownloadFailed: 'File download failed',
  agentFMListFailed: 'Failed to list directory',
  agentFMDeleteSuccess: 'Deleted successfully',
  agentFMDeleteFailed: 'Delete failed',
  agentFMDeleteConfirm: 'Are you sure you want to delete {name}? This action cannot be undone.',
  agentFMMkdirFailed: 'Failed to create directory',
  agentFMFileSizeLimit: 'File too large (max 50 MB), please use another transfer method'
}
